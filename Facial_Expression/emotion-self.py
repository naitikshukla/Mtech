##############################################################################################################
##		Script will show WEBCAM image with emotions emoji interpolated over face dynamically with position and size.
##		Also put text of emotion over frame.																##
##		Uses pre trained Fisher model : "models/emotion_detection_model.xml"								##
##		And Frontal Face detector: "models/haarcascade_frontalface_default.xml"								##
##		Uses Emotions image with all emotions from : "graphics/anger.JPG","graphics/sadness.JPG" etc		##
##		Created by : Naitik Shukla		On 17/8/2017	for Fulfillment of Mtech-KE from NUS-ISS(KE29)		##
##										Tested and Working for Python 2.7									##
##############################################################################################################
'''
REQUIREMENTS:
	* FOLDER Structure:
	...
	* FILES:
		...
		- emotion_detection_model.xml													(In models/)
		- neutral.JPG, anger.JPG, disgust.JPG, happy.JPG, sadness.JPG, surprise.JPG		(In graphics/)
	* WEBCAM
'''
import cv2
...
if __name__ == '__main__':
    ...
    #fisher_face.load('models/fish_model-demo.xml')      						#Load the demo model newly created
    fisher_face.load('models/fish_model-350.xml')      						#Load the earlier created model from models/*.xml file
    ...
    while read_value:                           #Iterate till camera pushes frame
        cv2.imshow("original", webcam_image)
        for norm_faces,(x,y,w,h) in find_faces(webcam_image):   #Repeat for all faces found in find_faces() function.
            ...
            cv2.putText(webcam_image,str(emo),(x,y),cv2.FONT_HERSHEY_SIMPLEX,2,(125,244,345))   #Show emotion on screen at location found for face.
            ...
            for c in range(0, 3):                           #For all 3 channel (RGB in opencv)
                webcam_image[y:y + h, x:x + w, c] = image_array[:, :, c] * .7 \
                                                    + webcam_image[y:y + h, x:x + w, c] * .3    #append the emotion image on original image with .8 alpha and .2 beta
